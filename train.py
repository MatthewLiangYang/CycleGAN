...
lr_scheduler_G = torch.optim.lr_scheduler.LambdaLR(opt_G, lr_lambda=LambdaLR(n_epoch,
                                                                      epoch,
                                                                      decay_epoch).step)
lr_scheduler_DA = torch.optim.lr_scheduler.LambdaLR(opt_DA, lr_lambda=LambdaLR(n_epoch,
                                                                      epoch,
                                                                      decay_epoch).step)

lr_scheduler_DB = torch.optim.lr_scheduler.LambdaLR(opt_DB, lr_lambda=LambdaLR(n_epoch,
                                                                      epoch,
                                                                      decay_epoch).step)
...
for epoch in range(n_epoch):
    for i, batch in enumerate(dataloader):
        ...
        # real_A = input_A.clone().detach().copy_(batch['A']).to(device)
        # real_B = input_B.clone().detach().copy_(batch['B']).to(device)
